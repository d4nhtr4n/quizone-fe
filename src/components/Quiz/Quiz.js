...
const answerTemplateTrueFalse = ['False', 'True'];
...
const Quiz = ({ question, index, currentIndex, onClick, onRemove, onDuplicate }) => {
    const seconds = () => {
      ...
      <div className={cx('info-container')}>
        <div className={cx('id')}>
          ...
          >
            ...
            <p>{question.type === 'quiz' ? 'Quiz' : 'True/False'}</p>
          </div>
        ...
        <div className={cx('mini-frame')}>
          ...
          <div className={cx('middle')}>
            {question.imageUri?.imgSrc ? (
              <img src={question.imageUri?.imgSrc} className={cx('image')} alt="Quest Thumbnail" />
            ) : (
          ...
          <div className={cx('answers')}>
            <Container>
              {question.type === 'quiz' ? (
                  ...
                  {Array.from({ length: 4 }, (_, index) => index + 1).map((_, index) => (
                    ...
                    >
                      <div className={cx('answer')}>
                        {question.answer === answerTemplateQuiz[index] && (
                ...
                <Row style={{ height: '40px' }}>
                  {Array.from({ length: 2 }, (_, index) => index + 1).map((_, index) => (
                    <Col xs={6} key={index}>
                      <div
                        className={cx('answer')}
                        style={{
                      ...
                      >
                        {question.answer === answerTemplateTrueFalse[index] && (
                          <span className={cx('correct')}></span>
                        )}
                      </div>
                    </Col>
